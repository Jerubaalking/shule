...
class Crud_model extends CI_Model {
	...
	// bEGINING OF VEHICLES
	  // Get vehicle list
    public function get_vehicle_list(){
      return $this->db->get_where('vehicles', array('id' => school_id()))->row_array();
    }
	public function get_vehicle_details($vehicle_id='', $column_name=''){
		// return $this->db->get_where('vehicles', array('id' => school_id()))->row_array();
		if($column_name != ''){
			return $this->db->get_where('vehicles', array('id' => $vehicle_id))->row($column_name);
		}else{
			return $this->db->get_where('vehicles', array('id' => $vehicle_id))->row_array();
		}
	  }
	...
	public function vehicle_update($param1 = '')
	{
		...
		$response = array(
			'status' => true,
			'notification' => get_phrase('vehicle_updated_succesfully')
		);
		return json_encode($response);
	}
	// delete vehicle
	public function vehicle_delete($param1 = '')
	{
		...
		$response = array(
			'status' => true,
			'notification' => get_phrase('vehicle_deleted_successfully')
		);
		return json_encode($response);
	}
	// END OF VEHICLES
	...
	public function get_bus_routes($params1 =''){
		return $this->db->get_where('bus_routes', array('id'=> school_id()))->row_array();
	}
	// Add bus_route
    public function add_bus_route(){
		$data['name'] = html_escape($this->input->post('name'));
		$data['roads'] = html_escape($this->input->post('roads'));
		$data['route_starts'] = html_escape($this->input->post('route_start'));
		$data['route_end'] = html_escape($this->input->post('route_end'));
		...
		$response = array(
		  'status' => true,
		  'notification' => get_phrase('bus_route_added_successfully')
		);
		  return json_encode($response);
	  }
// update bus route
	  public function update_bus_route($param1 = ''){
		$data['name'] = html_escape($this->input->post('name'));
		$data['roads'] = html_escape($this->input->post('roads'));
		$data['route_starts'] = html_escape($this->input->post('route_start'));
		$data['route_end'] = html_escape($this->input->post('route_end'));
		...
		$response = array(
		  'status' => true,
		  'notification' => get_phrase('bus_route_updated_successfully')
		);
		  return json_encode($response);
	  }
	   public function delete_bus_route($param1=''){
		$this->db->where('id', $param1);
		$this->db->delete('bus_routes');

		$response = array(
			'status' => true,
			'notification' => get_phrase('bus_route_deleted_successfully')
		);
		return json_encode($response);
	   }
	// END BUS-ROUTES
	...
	public function get_bonuses($params1 =''){
		return $this->db->get_where('bonuses', array('id'=> school_id()))->row_array();
	}
	// Add bonuses
    public function update_bonus($param1 = ''){
		$data['amount'] = html_escape($this->input->post('amount'));
		$data['name'] = html_escape($this->input->post('name'));
		$this->db->where('id', $param1);
		$this->db->update('bonuses');
  
		$response = array(
		  'status' => true,
		  'notification' => get_phrase('bonus_updated_successfully')
		);
		  return json_encode($response);
	  }
	  public function add_bonus(){
		...
		$response = array(
		  'status' => true,
		  'notification' => get_phrase('bonus_added_successfully')
		);
		  return json_encode($response);
	  }
	  public function delete_bonus($param1=''){
		$this->db->where('id', $param1);
		$this->db->delete('bonuses');

		$response = array(
			'status' => true,
			'notification' => get_phrase('bonus_deleted_successfully')
		);
		return json_encode($response);
	   }
	...
	public function get_salaries($params1 =''){
		return $this->db->get_where('salaries', array('id'=> school_id()))->row_array();
	}
	// Add salaries
    public function update_salary($param1 = ''){
		$data['amount'] = html_escape($this->input->post('amount'));
		$data['level'] = html_escape($this->input->post('level'));
		$data['ss_dedcution'] = html_escape($this->input->post('ss_deduction'));
		$data['take_home'] = html_escape($this->input->post('take_home'));
		$this->db->where('id', $param1);
		$this->db->update('salaries');
  
		$response = array(
		  'status' => true,
		  'notification' => get_phrase('salary_updated_successfully')
		);
		  return json_encode($response);
	  }
	  public function add_salary($param1 =''){
			$data['amount'] = html_escape($this->input->post('amount'));
			$data['school_id'] = $this->school_id;
			$data['session'] = $this->active_session;
			$data['employee_id'] = html_escape($this->input->post('employee_id'));
			$data['date_paid'] = html_escape($this->input->post('date_paid'));
			$data['month'] = html_escape($this->input->post('month'));
			$this->db->insert('salaries', $data);
			$deductions =$this->db->get_where('deductions', array('school_id'=>$school_id))->result_array();
				// get selected deduct from checkbox
				$deducts= html_escape($this->input->post('check_id'));	
				// alterate the arrays of the checkbox			
					 foreach($deducts as $key1 => $val) {
						//  get new save salary
						$salaries = $this->db->get_where('salaries', array('school_id'=>$data['school_id'],'employee_id'=>$data['employee_id'],'month'=>$data['month'],))->result_array();
						$numItems = count($salaries);
						$i = 0;
						// since its just one salary we use foreach salary and save the deduction
						foreach($salaries as $key=>$value) {
							if(++$i === $numItems) {
							$data2['salary_id']=$value['id'];
							$data2['deduction_id'] = $key1;
							$data2['school_id'] = $this->school_id;
							$data2['session'] = $this->active_session;
							$this->db->insert('deductions_chart', $data2);
						}
					}
					}
					$response = array(
						'status' => true,
						'notification' => get_phrase('salary_added_successfully')
					  );
					  
					  return json_encode($response);
	  }
	  public function delete_salary($param1=''){
		$deductions = $this->db->get_where('deductions_chart', array('salary_id' =>$param1))->result_array();

		$this->db->where('id', $param1);
		$this->db->delete('salaries');
		foreach($deductions as $deduction){
			$this->db->where('id', $deduction['id']);
			$this->db->delete('deductions_chart');
		}

		$response = array(
			'status' => true,
			'notification' => get_phrase('salary_deleted_successfully')
		);
		return json_encode($response);
	   }
	...
	// START DEDUCTION SECTION
	  public function get_deduction($param1=''){
		return $this->db->get_where('deductions', array('id'=> school_id()))->row_array();
	  }
	  public function update_deduction($param1=''){
		$data['name'] = html_escape($this->input->post('name'));
		$data['amount'] = html_escape($this->input->post('amount'));
		$data['percent'] = html_escape($this->input->post('percent'));
		$this->db->where('id', $param1);
		$this->db->update('deductions', $data);
  
		$response = array(
		  'status' => true,
		  'notification' => get_phrase('deduction_updated_successfully')
		);
		  return json_encode($response);
	  }
	  public function add_deduction(){
		...
		$response = array(
		  'status' => true,
		  'notification' => get_phrase('deduction_added_successfully')
		);
		  return json_encode($response);
	  }
	  public function delete_deduction($param1=''){
		$this->db->where('id', $param1);
		$this->db->delete('deductions');

		$response = array(
			'status' => true,
			'notification' => get_phrase('deduction_deleted_successfully')
		);
		return json_encode($response);
	   }
