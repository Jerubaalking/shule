...
class Driver extends CI_Controller {
	...
	public function section($action = "", $id = "") {
		...
		if ($action == 'list') {
			...
			$this->load->view('backend/driver/section/list', $page_data);
		}
	}
	//	SECTION ENDED
  ...
  public function invoice($param1 = "", $param2 = "") {
    ...
    if ($param1 == 'student') {
      ...
      $this->load->view('backend/driver/student/dropdown', $page_data);
    }
    ...
    if ($param1 == 'list') {
      ...
      $this->load->view('backend/driver/invoice/list', $page_data);
    }
  ...
  public function export($param1 = "", $date_from = "", $date_to = "", $selected_class = "", $selected_status = "") {
    ...
    if($param1 == 'pdf' || $param1 == 'print') {
      ...
      $html = $this->load->view('backend/driver/invoice/export',$page_data, true);
  ...
  function download_file($path, $name)
  {
	...
	public function expense_category($param1 = "", $param2 = "") {
		...
		if ($param1 == 'list') {
			$this->load->view('backend/driver/expense_category/list');
		}
		// showing the index file
		if(empty($param1)){
	...
	public function expense($param1 = "", $param2 = "") {
		...
		if ($param1 == 'list') {
			...
			$this->load->view('backend/driver/expense/list', $page_data);
		}
